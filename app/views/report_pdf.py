...
def report_pdf(request):
    if request.method == 'POST':
        ...
        if table_html:
            ...
            manual_col_widths = [40, 100, 200, 43, 70, 62, 62, 62, 62, 60]  # Adjust these values manually

            # Set font size for header
            c.setFont("Helvetica-Bold", 4.8)

            # Draw <thead> data only on the first page
            x_offset = 30
            y_offset = height - 150  # Start position for the header row
            header_cells = thead.find_all('th')
            for i, th in enumerate(header_cells):
                c.drawString(x_offset, y_offset, th.text.strip())
                c.rect(x_offset - 5, y_offset - 5, manual_col_widths[i], 20)
            ...
            for row in rows[1:]:  # Process rows from tbody
                ...
                # Track maximum row height for merged cells
                max_row_height = 15
                merged_cells = []  # Store merged cells to avoid duplicate drawings
                ...
                for i, cell in enumerate(cells):
                    ...
                    # Calculate width and height for the merged cell
                    ...
                    c.drawString(x_offset + 5, y_offset - 5, cell_text)

                    # Draw the outline for merged cells
                    c.rect(x_offset - 5, y_offset - 5, cell_width, -cell_height)
                    ...
                    # Store merged cell information
                    if rowspan > 1 or colspan > 1:
                        merged_cells.append({'row': row_count, 'col': i, 'colspan': colspan, 'rowspan': rowspan})

                # Move down based on max row height
                row_count += 1
                ...
                if row_count >= row_limit:
                    ...
                    # Do NOT redraw <thead> on subsequent pages; skip header rendering
